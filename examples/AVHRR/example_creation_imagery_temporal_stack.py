...
df = satbucket.read(
    ...
    # columns=["CHANNEL_1", "lon","lat", "time"],
...
list_ds_overpass = [
    overpass_to_dataset(df_overpass, x_dim="x", y_dim="y", x_index="scan_id", y_index="y")
...
for ds_swath in list_ds_overpass[0:2]:
    p = ds_swath["CHANNEL_1"].gpm.plot_map(add_swath_lines=False)
...
# ds_swath_remapped = ds_swath[["CHANNEL_1"]].gpm.remap_on(ds_aoi)
...
da = ds_stack["CHANNEL_1"].ffill(dim="time").compute()
