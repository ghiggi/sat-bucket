...
bucket_dir = "/home/ghiggi/data/F18_SSMIS_Bucket"

# Define area of interest
extent = [-85, -80, 50, 55] # [lon_min, lon_max, lat_min, lat_max]
...
list_ds_overpass = [overpass_to_dataset(df_overpass, 
                                        x_dim="along_track", y_dim="cross_track", 
                                        x_index="gpm_id", y_index="gpm_cross_track_id") for df_overpass in list_df_overpass]

###------------------------------------------------------------------------.
#### Plot satellite overpass
for ds_swath in list_ds_overpass[0:2]: 
    p = ds_swath["Tc_91V"].gpm.plot_map(add_swath_lines=False)
...
ds_aoi = satbucket.LonLatPartitioning(size=0.05, extent=extent).dataset_grid

# Define custom area of interest
# dst_crs = pyproj.CRS.from_epsg(6932)
# projection_extent = [x_min, x_max, y_min, y_max]
# ds_aoi = XYPartitioning(size=5_000, extent=projection_extent).dataset_grid(crs=dst_crs)

# Check remapping overpass onto common grid 
# ds_swath_remapped = ds_swath[["Tc_91V"]].gpm.remap_on(ds_aoi)    
...
da = ds_stack["Tc_91V"].ffill(dim="time").compute()
