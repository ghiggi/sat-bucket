...
# - Download 1C-SSMIS-F18 product with GPM-API:
...
if __name__ == "__main__": #  https://github.com/dask/distributed/issues/2520    
    ...
    bucket_dir = "/home/ghiggi/data/F18_SSMIS_Bucket_Granules"
    ...
    def create_dataframe_from_granule(filepath):
       #---------------------------------------------------------------------.
        ...
        geographic_extent = [80, 160, -80, -60]
        ...
        try:
            ...
            dict_isel_dicts = {scan_mode: dt[scan_mode].to_dataset().gpm.get_crop_slices_by_extent(extend_extent(geographic_extent, padding=2)) 
        ...
        dict_isel_dicts = {scan_mode: [isel_dict for isel_dict in list_isel_dicts
                           if get_slice_size(isel_dict["along_track"]) > 5] for scan_mode, list_isel_dicts in dict_isel_dicts.items()}
        n_slices_per_scan_mode = [len(v) for v in dict_isel_dicts.values()]
        n_slices = np.unique(n_slices_per_scan_mode)
        if len(n_slices) > 1: 
